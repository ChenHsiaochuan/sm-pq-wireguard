...
impl<T: Tun, B: UDP> WireGuard<T, B> {
    ...
    pub fn remove_peer(&self, pk_hash: &[u8; SIZE_HASH]) {
        let _ = self.peers.write().remove(pk_hash);
    }

    pub fn set_key(&self, sk: Option<(DhSecretKey, oqs::kem::SecretKey, oqs::kem::PublicKey)>) {
    ...
    pub fn get_sk(&self) -> Option<(DhSecretKey, oqs::kem::SecretKey, oqs::kem::PublicKey)> {
        self.peers
            .read()
            .get_sk()
            .map(|sk| (sk.0.clone(), sk.1.clone(), sk.2.clone()))
    }

    pub fn set_psk(&self, pk_hash: &[u8; SIZE_HASH], psk: [u8; 32]) -> bool {
        self.peers.write().set_psk(pk_hash, psk).is_ok()
    }
    pub fn get_psk(&self, pk_hash: &[u8; SIZE_HASH]) -> Option<[u8; 32]> {
        self.peers.read().get_psk(pk_hash).ok()
    }

    pub fn add_peer(&self, pk: DhPublicKey, pk_pq: &oqs::kem::PublicKey, pk_hash: &[u8; SIZE_HASH]) -> bool {
        let mut peers = self.peers.write();
        if peers.contains_key(&pk_hash) {
            return false;
        }
        ...
        let timers = Timers::new::<T, B>(self.clone(), &pk, &pk_pq , *enabled);
