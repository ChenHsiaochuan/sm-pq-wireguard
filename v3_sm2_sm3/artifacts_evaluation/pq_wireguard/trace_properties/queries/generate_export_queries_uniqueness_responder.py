...
with open('wireguard_uniqueness_responder_export_queries.spthy', 'w') as f:
	...
	f.write("query kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring, rb':bitstring, ra':bitstring, re':bitstring;"+'\n')
	f.write("event(eRConfirm(ck, kemltki, kemltkr, tpkr, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) && (event(eRConfirm(ck, kemltki, kemltkr, tpkr, kempeki', psk, h((rb, tpki)), h((ra', tpkr)), h(re'))))"+'\n')
	...
	for l in sets_list:
		...
		for j in range(0, len(l)):
			...
			query = "query kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring, rb':bitstring, ra':bitstring, re':bitstring"
