...
def res2dnf(arg1):
	with open(arg1) as infile, open(arg1+'.dnf', 'w') as newfile:
		...
		for line in Lines:
			...
			pattern = r"event\(eRKeys\(ck,kemltki,kemltkr,ldhi,ldhr,kempeki,dheki,dhekr,psk,h\(rb\),h\(ra\),h\(re\)"
			line = re.sub(pattern, "", line)
	...
	with open(arg1+'.dnf', 'r') as infile, open('results.cnfdnf', 'a') as resfile:
		data = infile.read()
		if(data == 'NULL'):
			resfile.write("DNF for agreement_rechello: \u2205\n")
			resfile.write("CNF for agreement_rechello: \u2205\n")
		else:
			...
			resfile.write("DNF for agreement_rechello: "+str(dnf)+"\n")
			resfile.write("CNF for agreement_rechello: "+str(cnf)+"\n")

res2dnf("process_read_access/agreement_rechello/wireguard_agreement_rechello.pv.log")
