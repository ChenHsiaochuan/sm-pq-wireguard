...
use crate::wireguard_hybrid::sm_crypto::{sm4_gcm_encrypt_inplace, Sm4Key};
use spin::Mutex;
    ...
    for SendJob<E, C, T, B>
{
    ...
    fn parallel_work(&self) {
        ...
        // encrypt body
        {
            ...
            // create a nonce object (12-byte: 4-byte zero prefix + 8-byte counter)
            let mut nonce = [0u8; 12];
            debug_assert_eq!(nonce.len(), CHACHA20_POLY1305.nonce_len());
            nonce[4..].copy_from_slice(header.f_counter.as_bytes());

            // encrypt contents of transport message using SM4-GCM,
            // fully in place (GFNI+AVX2 SM4 core, CLMUL GHASH; replaces
            // ring's ChaCha20-Poly1305). `packet` is body || 16-byte tag space.
            let sm4_key = Sm4Key(job.keypair.send.key);
            sm4_gcm_encrypt_inplace(&sm4_key, &nonce, &[], packet);
        }
