...
impl Timers {
    pub fn new<T: Tun, B: UDP>(
        ...
        pk: &DhPublicKey,       // public key of peer
        pk_pq: &oqs::kem::PublicKey, // PQ public key of peer
        running: bool,       // timers started
    ) -> Timers {
        macro_rules! fetch_peer {
            ( $wg:expr, $pk_hash:expr, $peer:ident) => {
                let peers = $wg.peers.read();
                let $peer = match peers.get(&$pk_hash) {
                    None => {
                        return;
                    }
                    Some(peer) => peer,
