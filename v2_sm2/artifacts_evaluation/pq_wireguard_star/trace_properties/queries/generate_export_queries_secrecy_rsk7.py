...
with open('wireguard_secrecy_rsk7_export_queries.spthy', 'w') as f:
	...
	f.write('query kemltki:bitstring, kemltkr:bitstring, kempeki:bitstring, dheki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring;'+'\n')
	f.write('event(eR_SK7(ck, kemltki, kemltkr, kempeki, psk, h(rb), h(ra), h(re))) && attacker(ck).'+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query kemltki:bitstring, kemltkr:bitstring, kempeki:bitstring, dheki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring"
			event ='==> '
