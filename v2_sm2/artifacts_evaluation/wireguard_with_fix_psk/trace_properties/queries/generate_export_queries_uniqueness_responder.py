...
with open('wireguard_uniqueness_responder_export_queries.spthy', 'w') as f:
	...
	f.write("query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, dheki':bitstring, dhekr':bitstring, psk:bitstring, ck:bitstring;"+'\n')
	f.write("event(eRConfirm(ck, ldhi, ldhr, dheki, dhekr, psk)) && event(eRConfirm(ck, ldhi, ldhr, dheki', dhekr', psk))"+'\n')
	f.write("==> ((dheki = dheki') && (dhekr = dhekr'))."+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, dheki':bitstring, dhekr':bitstring, psk:bitstring, ck:bitstring"
			event ="==> ((dheki = dheki') && (dhekr = dhekr')) || "
