...
with open('wireguard_agreement_inithello_export_queries.spthy', 'w') as f:
	...
	f.write('event(eRRec(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re)))'+'\n')
	f.write('==> event(eISend(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, psk, h(rb))).'+'\n\n')
	...
	for l in sets_list:
		...
		for j in range(0, len(l)):
			...
			event ='==> event(eISend(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, psk, h(rb))) || '
