...
Ru, Rv, Rx, Ry, Rc, Ki, Kr, Kx, Ra, Rb, Re, Rs = symbols("Ru, Rv, Rx, Ry, Rc, Ki, Kr, Kx, Ra, Rb, Re, Rs")
...
#			pattern = r"ck,kemltki,kemltkr,ldhi,ldhr,kempeki,dheki,psk"
#			line = re.sub(pattern, "", line)

			pattern = r"event\(eISend\(ck,kemltki,kemltkr,ldhi,ldhr,kempeki,dheki,psk,h\(rb\)\)"
			line = re.sub(pattern, "", line)
	...
	with open(arg1+'.dnf', 'r') as infile, open('results.cnfdnf', 'a') as resfile:
		data = infile.read()
		if(data == 'NULL'):
			resfile.write("DNF for agreement_inithello: \u2205\n")
			resfile.write("CNF for agreement_inithello: \u2205\n")
		else:
			...
			resfile.write("DNF for agreement_inithello: "+str(dnf)+"\n")
			resfile.write("CNF for agreement_inithello: "+str(cnf)+"\n")

res2dnf("process_read_access/agreement_inithello/wireguard_agreement_inithello.pv.log")
