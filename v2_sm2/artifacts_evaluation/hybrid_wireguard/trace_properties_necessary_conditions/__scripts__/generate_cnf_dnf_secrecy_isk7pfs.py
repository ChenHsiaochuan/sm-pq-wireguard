...
def res2dnf(arg1):
	...
	with open(arg1+'.dnf', 'r') as infile, open('results.cnfdnf', 'a') as resfile:
		data = infile.read()
		if(data == 'NULL'):
			resfile.write("DNF for secrecy_isk7pfs: \u2205\n")
			resfile.write("CNF for secrecy_isk7pfs: \u2205\n")
		else:
			cnf = to_cnf(data, simplify=False, force=False)
			dnf = to_dnf(cnf, simplify=True, force=True)
			for old, new in paper_table:
				dnf = re.sub(rf"\b{old}\b", new, str(dnf))
				cnf = re.sub(rf"\b{old}\b", new, str(cnf))
			resfile.write("DNF for secrecy_isk7pfs: "+str(dnf)+"\n")
			resfile.write("CNF for secrecy_isk7pfs: "+str(cnf)+"\n")

res2dnf("process_read_access/secrecy_isk7pfs/wireguard_secrecy_isk7pfs.pv.log")
