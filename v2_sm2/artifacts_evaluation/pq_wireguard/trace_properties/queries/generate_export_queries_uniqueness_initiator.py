...
with open('wireguard_uniqueness_initiator_export_queries.spthy', 'w') as f:
	...
	f.write("event(eIConfirm(ck, kemltki, kemltkr, tpki, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) && (event(eIConfirm(ck, kemltki, kemltkr, tpki, kempeki', psk, h((rb', tpki)), h((ra, tpkr)), h(re))))"+'\n')
	f.write("==> ((kempeki = kempeki') && (rb = rb'))."+'\n\n')
