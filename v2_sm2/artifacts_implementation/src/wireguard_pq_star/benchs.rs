...
use crate::wireguard_pq_star::handshake::crypto_params::{EPHEMERAL_KEM_ALG, SIZE_HASH, STATIC_KEM_ALG};
...
fn setup_devices<R: RngCore + CryptoRng, O: Default>(
    ...
    rng2: &mut R,
    rng3: &mut R,
) -> (oqs::kem::PublicKey, [u8; SIZE_HASH], Device<O>, oqs::kem::PublicKey, [u8; SIZE_HASH], Device<O>) {
    ...
    let hash1 = Device::<O>::hash_static_keys(&pk1_pq);

    let (pk2_pq, sk2_pq) = kemalg.keypair().unwrap();

    let hash2 = Device::<O>::hash_static_keys(&pk2_pq);
    ...
    dev1.set_sk(Some((sk1_pq, pk1_pq.clone())));
    dev2.set_sk(Some((sk2_pq, pk2_pq.clone())));

    dev1.add(&pk2_pq, O::default()).unwrap();
    dev2.add(&pk1_pq, O::default()).unwrap();

    dev1.set_psk(&hash2, psk).unwrap();
    dev2.set_psk(&hash1, psk).unwrap();

    (pk1_pq, hash1, dev1, pk2_pq, hash2, dev2)
}
...
pub fn benchmark_pq_star_handshake(nb_iter: u32){
    let (pk1_pq, pk1_hash, mut dev1, pk2_pq, pk2_hash, mut dev2): (_, _, Device<usize>, _, _, _) =
        setup_devices(&mut OsRng, &mut OsRng, &mut OsRng);

    // do a few handshakes (every handshake should succeed)

    println!("PQ-WireGuard*: (static kem: {}, ephemeral kem: {})", STATIC_KEM_ALG, EPHEMERAL_KEM_ALG);
