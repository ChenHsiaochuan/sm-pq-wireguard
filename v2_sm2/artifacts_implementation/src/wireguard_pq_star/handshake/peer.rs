...
use oqs;
use crate::wireguard_pq_star::handshake::crypto_params::{HASH, SIZE_HASH, SIZE_STATIC_KEM_PUB_KEY};
...
impl<O> Peer<O> {
    pub fn new(pk_pq: oqs::kem::PublicKey, own_pk_pq: &oqs::kem::PublicKey, opaque: O) -> Self {
        Self {
            ...
            psk: [0u8; 32],
            pk_hash: HASH!(<[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into(),
            pk_pq: pk_pq,
            k_send: Mutex::new(0),
            k_recv: Mutex::new(0),
        }
