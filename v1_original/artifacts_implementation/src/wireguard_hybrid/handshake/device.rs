...
impl<O> Device<O> {
    ...
    pub fn hash_static_keys(pk: &PublicKey, pk_pq: &oqs::kem::PublicKey) -> [u8; SIZE_HASH] {
        HASH!(pk.as_bytes(), <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into()
    }
...
impl<O> Device<O> {
    ...
    pub fn add(&mut self, pk: &PublicKey, pk_pq: &oqs::kem::PublicKey ,opaque: O) -> Result<(), ConfigError> {
        ...
        self.pk_map.insert(
            ...
            Peer::new(
                ...
                self.keyst
                    .as_ref()
                    .map(|key| {
                        *key.sk.diffie_hellman(pk).as_bytes()
                    })
                    .unwrap_or([0u8; 32]),
                opaque,
            ),
