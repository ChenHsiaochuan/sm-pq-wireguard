...
type TemporaryState = (u32, PublicKey, oqs::kem::PublicKey ,GenericArray<u8, U32>, GenericArray<u8, U32>);
...
pub(super) fn create_initiation<R: RngCore + CryptoRng, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        concat[SIZE_X25519_POINT..SIZE_X25519_POINT+SIZE_HASH].copy_from_slice(&peer.hash_static_pq_send);
...
pub(super) fn consume_initiation_second_part<'a, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        *peer.state.lock() = State::Reset;
        ...
        let mut concat = [0u8; SIZE_X25519_POINT + SIZE_HASH + SIZE_PSK];
        concat[..SIZE_X25519_POINT].copy_from_slice(&peer.ss);
        concat[SIZE_X25519_POINT..SIZE_X25519_POINT+SIZE_HASH].copy_from_slice(&peer.hash_static_pq_recv);
        concat[SIZE_X25519_POINT+SIZE_HASH..].copy_from_slice(peer.psk.as_bytes());
        let (ck, key) = KDF2!(&ck, &concat);
...
pub(super) fn create_response<R: RngCore + CryptoRng, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        concat_k[..SIZE_X25519_POINT].copy_from_slice(shared_secret(&eph_sk, &eph_r_pk)?.as_bytes());
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        let (hs, ck, local, eph_sk, eph_sk_pq) = match *peer.state.lock() {
            State::InitiationSent {
                ...
                ref eph_sk,
                ref eph_sk_pq,
            } => Ok((hs, ck, local, StaticSecret::from(eph_sk.to_bytes()), kemalg.secret_key_from_bytes(eph_sk_pq.as_ref()).unwrap().to_owned())),
