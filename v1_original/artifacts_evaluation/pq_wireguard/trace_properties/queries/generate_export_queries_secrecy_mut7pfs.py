...
with open('wireguard_secrecy_mut7pfs_export_queries.spthy', 'w') as f:
	...
	f.write('event(eRConfirm(ck, kemltki, kemltkr, tpkr, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) && event(eIConfirm(ck, kemltki, kemltkr, tpki, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) && attacker(ck).'+'\n\n')
