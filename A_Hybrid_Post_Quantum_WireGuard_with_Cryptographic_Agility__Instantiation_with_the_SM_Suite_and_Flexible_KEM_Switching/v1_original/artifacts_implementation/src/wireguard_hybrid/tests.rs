...
fn test_pure_wireguard() {
    ...
    let pk1_hash = Device::<u32>::hash_static_keys(&pk1, &pk1_pq);
    let pk2_hash = Device::<u32>::hash_static_keys(&pk2, &pk2_pq);

    wg1.set_key(Some((sk1, sk1_pq.clone(), pk1_pq.clone())));
    wg2.set_key(Some((sk2, sk2_pq.clone(), pk2_pq.clone())));

    wg1.add_peer(pk2, &pk2_pq, &pk2_hash);
    wg2.add_peer(pk1, &pk1_pq, &pk1_hash);
