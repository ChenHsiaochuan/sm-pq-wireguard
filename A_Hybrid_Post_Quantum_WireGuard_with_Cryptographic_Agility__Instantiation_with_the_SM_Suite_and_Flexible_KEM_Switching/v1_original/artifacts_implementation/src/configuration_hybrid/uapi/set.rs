...
use crate::wireguard_hybrid::handshake::crypto_params::{SIZE_STATIC_KEM_PUB_KEY, SIZE_STATIC_KEM_SECRET_KEY, STATIC_KEM_ALG};
...
impl<'a, C: Configuration> LineParser<'a, C> {
    ...
    fn new_peer(value: &str) -> Result<ParserState, ConfigError> {
        ...
        match Vec::from_hex(value) {
            Ok(pkv) => Ok({
                let k: [u8; 32+SIZE_STATIC_KEM_PUB_KEY] = <[u8; 32+SIZE_STATIC_KEM_PUB_KEY]>::try_from(pkv).unwrap();
                let pk: [u8; 32] = k[..32].try_into().unwrap();
                let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[32..].try_into().unwrap();
        ...
        match self.state {
            ...
            ParserState::Interface => match key {
                ...
                "private_key" => match Vec::from_hex(value) {
                    Ok(skv) => {
                        let k = <[u8; 32+SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]>::try_from(skv).unwrap();

                        self.config.set_private_key(if k.ct_eq(&[0u8; 32+SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]).into() {
                            None
                        } else {
                            let sk: [u8; 32] = k[..32].try_into().unwrap();
                            let sk_pq: [u8; SIZE_STATIC_KEM_SECRET_KEY] = k[32..32+SIZE_STATIC_KEM_SECRET_KEY].try_into().unwrap();
                            let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[32+SIZE_STATIC_KEM_SECRET_KEY..].try_into().unwrap();
