...
use crate::wireguard_hybrid::handshake::crypto_params::SIZE_HASH;
use super::udp::Owner;
use super::*;
...
pub struct PeerState {
    ...
    pub public_key_pq: oqs::kem::PublicKey,
...
pub trait Configuration {
    ...
    fn set_private_key(&self, sk: Option<(StaticSecret, oqs::kem::SecretKey, oqs::kem::PublicKey)>);
    ...
    fn get_private_key(&self) -> Option<(StaticSecret, oqs::kem::SecretKey, oqs::kem::PublicKey)>;
    ...
    fn remove_peer(&self, pk_hash: &[u8; SIZE_HASH]);
    ...
    fn add_peer(&self, pk: PublicKey, pk_pq: &oqs::kem::PublicKey, pk_hash: &[u8; 32]) -> bool;
    ...
    fn set_preshared_key(&self, pk_hash: &[u8; SIZE_HASH], psk: [u8; 32]);
    ...
    /// - `peer': The public key of the peer
    /// - `psk`
    fn set_endpoint(&self, pk_hash: &[u8; SIZE_HASH], addr: SocketAddr);
    ...
    /// - `peer': The public key of the peer
    /// - `psk`
    fn set_persistent_keepalive_interval(&self, pk_hash: &[u8; SIZE_HASH], secs: u64);
    ...
    ///
    /// An error if no such peer exists
    fn replace_allowed_ips(&self, pk_hash: &[u8; SIZE_HASH]);
    ...
    fn add_allowed_ip(&self, pk_hash: &[u8; SIZE_HASH], ip: IpAddr, masklen: u32);
...
impl<T: tun::Tun, B: udp::PlatformUDP> Configuration for WireGuardConfig<T, B> {
    ...
    fn remove_peer(&self, pk_hash: &[u8; SIZE_HASH]) {
        self.lock().wireguard.remove_peer(pk_hash);
    }

    fn add_peer(&self, pk: PublicKey, pk_pq: &oqs::kem::PublicKey, pk_hash: &[u8; 32]) -> bool {
        self.lock().wireguard.add_peer(pk, pk_pq, pk_hash)
    }

    fn set_preshared_key(&self, pk_hash: &[u8; 32], psk: [u8; 32]) {
        self.lock().wireguard.set_psk(pk_hash, psk);
    }

    fn set_endpoint(&self, pk_hash: &[u8; 32], addr: SocketAddr) {
        if let Some(peer) = self.lock().wireguard.peers.read().get(pk_hash) {
            peer.set_endpoint(B::Endpoint::from_address(addr));
        }
    }

    fn set_persistent_keepalive_interval(&self, pk_hash: &[u8; 32], secs: u64) {
        if let Some(peer) = self.lock().wireguard.peers.read().get(pk_hash) {
            peer.opaque().set_persistent_keepalive_interval(secs);
        }
    }

    fn replace_allowed_ips(&self, pk_hash: &[u8; 32]) {
        if let Some(peer) = self.lock().wireguard.peers.read().get(pk_hash) {
            peer.remove_allowed_ips();
        }
    }

    fn add_allowed_ip(&self, pk_hash: &[u8; 32], ip: IpAddr, masklen: u32) {
        if let Some(peer) = self.lock().wireguard.peers.read().get(pk_hash) {
        ...
        for (pk, p) in peers.iter() {
            ...
            if let Some(psk) = cfg.wireguard.get_psk(&pk) {
                ...
                state.push(PeerState {
                    ...
                    last_handshake_time,
                    public_key: p.pk.clone(),
                    public_key_pq: p.pk_pq.clone(),
                })
            }
        }
        state
