...
with open('wireguard_uniqueness_initiator_export_queries.spthy', 'w') as f:
	...
	f.write("set nounifIgnoreNtimes = 4."+"\n")
	f.write("noselect x:bitstring; attacker(exp(g,x))."+"\n")
	f.write("noselect x:bitstring; attacker(h(x))."+"\n")
	f.write("noselect x, y: bitstring; attacker(senc( *x, h( *y)))[ignoreAFewTimes]."+"\n\n")			

	f.write("query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ldhi':bitstring, ldhr':bitstring, dheki':bitstring, dhekr':bitstring, kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, psk':bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring, rb':bitstring, ra':bitstring, re':bitstring;"+'\n')
	f.write("event(eIConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))) && event(eIConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki', dheki', dhekr', psk, h(rb'), h(ra'), h(re')))"+'\n')
	f.write("==> ((dheki = dheki') && (dhekr = dhekr') && (kempeki = kempeki') && (rb = rb') && (ra = ra') && (re = re'))."+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ldhi':bitstring, ldhr':bitstring, dheki':bitstring, dhekr':bitstring, kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, psk':bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring, rb':bitstring, ra':bitstring, re':bitstring,"
			event ="==> ((dheki = dheki') && (dhekr = dhekr') && (kempeki = kempeki') && (rb = rb') && (ra = ra') && (re = re')) || "
			for i in range(1, len(l[0])+1):
				
				if (l == S1c) or (i == len(l[0])):
					#query +=' j'+str(i)+':time'
					event +='(event(eRevPri('+l[j][i-2]+'))@j'+str(i)+')'
				else:
					#query +=' j'+str(i)+':time, '
					event +='(event(eRevPri('+l[j][i-2]+'))@j'+str(i)+') || '
			f.write(query+';\n')
...
with open('wireguard_uniqueness_initiator_export_queries.spthy', 'r') as f:
	fdata = f.read()
