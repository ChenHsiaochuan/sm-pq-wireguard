    ...
    pub fn process<'a, R: RngCore + CryptoRng>(
        ...
        match LittleEndian::read_u32(msg) {
            TYPE_INITIATION => {
                ...
                let (peer, pk_hash , st_intermediate) = noise::consume_initiation_first_part(self, keyst, &msg.noise)?;
                peer.macs_validator.lock().check_mac1(msg.noise.as_bytes(), &msg.macs)?;
                ...
                if let Some(src) = src {
                    // check mac2 field
                    if !peer.macs_validator.lock().check_mac2(msg.noise.as_bytes(), &src, &msg.macs) {
                ...
                // let now = Instant::now();
                let st = noise::consume_initiation_second_part(self, &msg.noise, st_intermediate, peer)?;
                ...
                let keys = noise::create_response(rng, peer, local, st, &mut resp.noise)
                    .map_err(|e| {
                        self.release(local);
                        e
                    })?;
