...
with open('wireguard_uniqueness_initiator_export_queries.spthy', 'w') as f:
	...
	f.write("(event(eIConfirm(ck, kemltki, kemltkr, kempeki, psk, h(rb), h(ra), h(re)))) && (event(eIConfirm(ck, kemltki, kemltkr, kempeki', psk, h(rb'), h(ra'), h(re'))))"+'\n')
	f.write("==> ((kempeki = kempeki') && (rb = rb') && (ra = ra') && (re = re'))."+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query kemltki:bitstring, kemltkr:bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring, rb':bitstring, ra':bitstring, re':bitstring,"
			event ="==> ((kempeki = kempeki') && (rb = rb') && (ra = ra') && (re = re')) || "
