...
lemma_header_secrecy_rsk7pfs = """lemma Secrecy_RSK7PFS:\nall-traces\n\"\nAll #i #j kemltki kemltkr ldhi ldhr kempeki dheki dhekr psk rb ra re ck.\n(RConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))@i & K(ck)@j & not(Ex #j1. RevPsk(psk)@j1 & (#j1 < #i)) & not(Ex #j2. RevLDH(ldhr)@j2 & (#j2 < #i)) &
...
extract_and_write_expression(input_filename, "secrecy_rsk7pfs.dnf", "secrecy_rsk7pfs")


generate_tamarin_lemma_secrecy_rsk7pfs("secrecy_rsk7pfs.dnf", variable_mapping_secrecy, "secrecy_rsk7pfs.lemma")