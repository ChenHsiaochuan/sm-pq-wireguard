...
with open('wireguard_unilateral_initiator_export_queries.spthy', 'w') as f:
	...
	f.write("")
	f.write("")
	f.write("query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ldhr':bitstring, psk:bitstring, ck:bitstring;"+'\n')
	f.write("event(eRConfirm(ck, ldhi, ldhr, dheki, dhekr, psk)) && event(eHonestK(ldhi)) && event(eHonestK(ldhr)) && event(eHonestpsk(psk)) && event(eIConfirm(ck, ldhi, ldhr', dheki, dhekr, psk))"+'\n')
	f.write("==> (ldhr = ldhr')."+'\n\n')
	...
	for l in sets_list:
		...
		for j in range(0, len(l)):
			...
			query = "query ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ldhr':bitstring, psk:bitstring, ck:bitstring"
			event ="==> (ldhr = ldhr') || "
