...
with open('wireguard_agreement_rechello_export_queries.spthy', 'w') as f:
	...
	f.write('query i:time,j:time, kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring;'+'\n')
	f.write('event(eIKeys(ck, kemltki, kemltkr, tpki, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re)))'+'\n')
	f.write('==> event(eRKeys(ck, kemltki, kemltkr, tpkr, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))).'+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query i:time,j:time, kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring"
			event ='==> event(eRKeys(ck, kemltki, kemltkr, tpkr, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) || '
