...
use super::handshake::{Device, MAX_HANDSHAKE_MSG_SIZE};
...
pub enum HandshakeJob<E> {
    Message(Vec<u8>, E),
    New(DhPublicKey, oqs::kem::PublicKey),
}
...
pub fn handshake_worker<T: Tun, B: UDP>(
    ...
    for job in rx {
        ...
        match job {
            ...
            HandshakeJob::New(pk, pk_pq) => {
                let pk_hash = Device::<u32>::hash_static_keys(&pk, &pk_pq);
                if let Some(peer) = wg.peers.read().get(&pk_hash) {
                    debug!(
                        "{} : handshake worker, new handshake requested for {}",
                        wg, peer
                    );
                    let device = wg.peers.read();
                    let _ = device.begin(&mut OsRng, &pk_hash).map(|msg| {
