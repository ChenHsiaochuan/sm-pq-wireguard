...
use crate::wireguard_hybrid::handshake::crypto_params::{SIZE_SM2_POINT, SIZE_STATIC_KEM_PUB_KEY, SIZE_STATIC_KEM_SECRET_KEY};
...
pub fn serialize<C: Configuration, W: io::Write>(writer: &mut W, config: &C) -> io::Result<()> {
    ...
    config
        ...
        .map(|sk| {
            ...
            // SM2 secret key scalar is 32 bytes; use SIZE_SM2_POINT-1 = 32
            let mut concat_pk = [0u8; (SIZE_SM2_POINT-1) + SIZE_STATIC_KEM_SECRET_KEY + SIZE_STATIC_KEM_PUB_KEY];
            concat_pk[..SIZE_SM2_POINT-1].copy_from_slice(&sk.0.to_bytes());
            concat_pk[SIZE_SM2_POINT-1..SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY].copy_from_slice(&bytes_sk_pq);
            concat_pk[SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY..].copy_from_slice(&bytes_pk_pq);
    ...
    while let Some(p) = peers.pop() {
        ...
        let bytes_pk_pq = <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(p.public_key_pq.as_ref()).unwrap();
        let mut concat_pk = [0u8; SIZE_SM2_POINT + SIZE_STATIC_KEM_PUB_KEY];
        concat_pk[..SIZE_SM2_POINT].copy_from_slice(p.public_key.to_encoded_point(true).as_bytes());
        concat_pk[SIZE_SM2_POINT..].copy_from_slice(&bytes_pk_pq);
