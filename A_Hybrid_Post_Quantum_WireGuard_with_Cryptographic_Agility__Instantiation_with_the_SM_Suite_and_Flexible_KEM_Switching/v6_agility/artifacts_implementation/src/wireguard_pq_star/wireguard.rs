...
impl<T: Tun, B: UDP> WireGuard<T, B> {
    ...
    pub fn set_key(&self, sk: Option<(oqs::kem::SecretKey, oqs::kem::PublicKey)>) {
    ...
    pub fn add_peer(&self, pk_pq: &oqs::kem::PublicKey, pk_hash: &[u8; SIZE_HASH]) -> bool {
        let mut peers = self.peers.write();
        ...
        let timers = Timers::new::<T, B>(self.clone(), &pk_pq , *enabled);
