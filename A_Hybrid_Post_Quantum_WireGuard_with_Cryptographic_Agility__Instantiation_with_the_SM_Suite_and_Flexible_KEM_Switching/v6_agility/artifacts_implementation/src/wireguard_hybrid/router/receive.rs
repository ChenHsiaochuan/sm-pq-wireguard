...
use crate::wireguard_hybrid::sm_crypto::{sm4_gcm_decrypt_inplace, Sm4Key};
use spin::Mutex;
    ...
    fn parallel_work(&self) {
        ...
        // decrypt
        {
            ...
            let ok = (|| {
                ...
                // create nonce object (12-byte: 4-byte zero prefix + 8-byte counter)
                let mut nonce = [0u8; 12];
                debug_assert_eq!(nonce.len(), CHACHA20_POLY1305.nonce_len());
                nonce[4..].copy_from_slice(header.f_counter.as_bytes());

                // attempt to open (and authenticate) the body using SM4-GCM,
                // decrypting in place (GFNI+AVX2 SM4 core, CLMUL GHASH).
                let sm4_key = Sm4Key(job.state.keypair.recv.key);
                let plaintext_len = match sm4_gcm_decrypt_inplace(&sm4_key, &nonce, &[], packet) {
                    Ok(n) => n,
                    Err(_) => return false,
                };
                // zero-pad the former tag region so downstream length checks
                // (and key-confirmation keep-alives) see clean trailing bytes.
                for b in &mut packet[plaintext_len..] {
                    *b = 0;
                }
