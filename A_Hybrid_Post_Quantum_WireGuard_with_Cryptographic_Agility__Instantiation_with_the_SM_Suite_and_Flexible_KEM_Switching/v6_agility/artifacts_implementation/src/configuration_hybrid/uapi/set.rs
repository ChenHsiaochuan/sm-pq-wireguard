...
use crate::wireguard_hybrid::handshake::crypto_params::{SIZE_SM2_POINT, SIZE_STATIC_KEM_PUB_KEY, SIZE_STATIC_KEM_SECRET_KEY, STATIC_KEM_ALG};
...
impl<'a, C: Configuration> LineParser<'a, C> {
    ...
    fn new_peer(value: &str) -> Result<ParserState, ConfigError> {
        ...
        match Vec::from_hex(value) {
            Ok(pkv) => Ok({
                let k: [u8; SIZE_SM2_POINT+SIZE_STATIC_KEM_PUB_KEY] = <[u8; SIZE_SM2_POINT+SIZE_STATIC_KEM_PUB_KEY]>::try_from(pkv).unwrap();
                let pk: [u8; SIZE_SM2_POINT] = k[..SIZE_SM2_POINT].try_into().unwrap();
                let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[SIZE_SM2_POINT..].try_into().unwrap();
                ParserState::Peer(ParsedPeer {
                public_key: DhPublicKey::from_sec1_bytes(&pk).unwrap(),
                public_key_pq: kemalg_static.public_key_from_bytes(&pk_pq).unwrap().to_owned(),
                remove: false,
                update_only: false,
                allowed_ips: vec![],
                preshared_key: None,
    ...
    pub fn parse_line(&mut self, key: &str, value: &str) -> Result<(), ConfigError> {
        ...
        fn flush_peer<C: Configuration>(config: &C, peer: &ParsedPeer) -> Option<ConfigError> {
            ...
            if peer.remove {
                ...
                let pk_hash = Device::<u32>::hash_static_keys(&peer.public_key, &peer.public_key_pq);
        ...
        match self.state {
            ...
            ParserState::Interface => match key {
                ...
                "private_key" => match Vec::from_hex(value) {
                    Ok(skv) => {
                        let k = <[u8; SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]>::try_from(skv).unwrap();

                        self.config.set_private_key(if k.ct_eq(&[0u8; SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]).into() {
                            None
                        } else {
                            let sk: [u8; SIZE_SM2_POINT-1] = k[..SIZE_SM2_POINT-1].try_into().unwrap();
                            let sk_pq: [u8; SIZE_STATIC_KEM_SECRET_KEY] = k[SIZE_SM2_POINT-1..SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY].try_into().unwrap();
                            let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[SIZE_SM2_POINT-1+SIZE_STATIC_KEM_SECRET_KEY..].try_into().unwrap();

                            let kemalg_static = oqs::kem::Kem::new(STATIC_KEM_ALG).unwrap();
                            Some((
                                DhSecretKey::from_bytes(&sk.into()).unwrap(),
                                kemalg_static.secret_key_from_bytes(&sk_pq).unwrap().to_owned(),
                                kemalg_static.public_key_from_bytes(&pk_pq).unwrap().to_owned(),
                            ))
                        });
                        Ok(())
                    }
                ...
                "replace_peers" => match value {
                    "true" => {
                        for p in self.config.get_peers() {
                            let pk_hash = Device::<u32>::hash_static_keys(&p.public_key, &p.public_key_pq);
