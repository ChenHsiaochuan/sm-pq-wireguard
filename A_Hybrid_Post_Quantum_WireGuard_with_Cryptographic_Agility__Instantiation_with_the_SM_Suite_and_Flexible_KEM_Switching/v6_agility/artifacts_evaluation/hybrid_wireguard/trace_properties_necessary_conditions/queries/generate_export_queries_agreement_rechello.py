...
with open('wireguard_agreement_rechello_export_queries.spthy', 'w') as f:
	...
	f.write('event(eIKeys(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re)))'+'\n')
	f.write('==> event(eRKeys(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))).'+'\n\n')
	...
	for l in sets_list:
		...
		for j in range(0, len(l)):
			...
			event ='==> event(eRKeys(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))) || '
