...
replace_string_in_files("./resolve_simple", "in(att,(=sxI, (sidI_1:bitstring, (kempekI_1:bitstring, (ct1_1:bitstring, (astat_1:bitstring, (ats_1:bitstring, macI1_1:bitstring)))))));", "in(att,(=sxI, (sidI_1:bitstring, (kempekI_1:bitstring, (ct1_1:bitstring, (astat_1:bitstring, (ats_1:bitstring, macI1_1:bitstring)))))));\n\t\t\tnew stp1_1[]:bitstring;\n\t\t\tevent eTest( stp1_1, kempekI_1 );\n\n")
replace_string_in_files("./resolve_simple", "reduc forall k:bitstring, m:bitstring;   checkmac(k, m, mac(k, m)) = okay.", "reduc forall k:bitstring, m:bitstring;   checkmac(k, m, mac(k, m)) = okay.\naxiom x: bitstring, y: bitstring, z: bitstring; event(eTest(x, y)) && event(eTest(x, z)) ==> y = z.\n\n")
replace_string_in_files("./resolve_simple", "event eRevRe(bitstring).\nevent eTest(bitstring,bitstring).\n", "event eRevRe(bitstring).\n")
