...
type TemporaryStateIntermediate = (GenericArray<u8, U32>, GenericArray<u8, U32>);

type TemporaryState = (u32, oqs::kem::PublicKey ,GenericArray<u8, U32>, GenericArray<u8, U32>);
...
pub(super) fn create_initiation<R: RngCore + CryptoRng, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        let ck = KDF1!(&ck, &eph_pk_pq_bytes);
        ...
        // H := HASH(H, msg.ephemeral_pq)

        let hs = HASH!(&hs, msg.f_ephemeral_pq);

        // (C, k) := Kdf2(C, shk1)

        let bytes = <[u8; SIZE_STATIC_KEM_SHARED_SECRET]>::try_from(shk1.as_ref()).unwrap();
        let (ck, key) = KDF2!(&ck, &bytes);
        ...
        // (C, k) := Kdf2(C, HASH(spki_pq || spkr_pq) || psk)

        let mut concat = [0u8; SIZE_HASH + SIZE_PSK];
        concat[..SIZE_HASH].copy_from_slice(&peer.hash_static_pq_send);
        concat[SIZE_HASH..].copy_from_slice(peer.psk.as_bytes());
...
pub(super) fn consume_initiation_first_part<'a, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        let ck = KDF1!(&ck, &msg.f_ephemeral_pq);
...
pub(super) fn consume_initiation_second_part<'a, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        *peer.state.lock() = State::Reset;
        ...
        let mut concat = [0u8; SIZE_HASH + SIZE_PSK];
        concat[..SIZE_HASH].copy_from_slice(&peer.hash_static_pq_recv);
        concat[SIZE_HASH..].copy_from_slice(peer.psk.as_bytes());
        let (ck, key) = KDF2!(&ck, &concat);
        ...
        Ok((msg.f_sender.get(), eph_r_pk_pq, hs, ck))
    })
}
...
pub(super) fn create_response<R: RngCore + CryptoRng, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        let (receiver, eph_r_pk_pq, hs, ck) = state;
        ...
        // C := Kdf1(C, msg.ephemeral_ct_pq)

        let ck = KDF1!(&ck, &eph_ct_pq_bytes);

        // H := Hash(H || msg.ephemeral_ct_pq)

        let hs = HASH!(&hs, &msg.f_ephemeral_ct_pq);

        // C := Kdf1(C, shk2)

        let shk2_bytes = <[u8; SIZE_EPHEMERAL_KEM_SHARED_SECRET]>::try_from(shk2.as_ref()).unwrap();

        let ck = KDF1!(&ck, &shk2_bytes);

        // C := Kdf1(C, shk3)

        let shk3_bytes = <[u8; SIZE_STATIC_KEM_SHARED_SECRET]>::try_from(shk3.as_ref()).unwrap();

        let ck = KDF1!(&ck, &shk3_bytes);
...
pub(super) fn consume_response<'a, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        let (hs, ck, local, eph_sk_pq) = match *peer.state.lock() {
            State::InitiationSent {
                hs,
                ck,
                local,
                ref eph_sk_pq,
            } => Ok((hs, ck, local, kemalg.secret_key_from_bytes(eph_sk_pq.as_ref()).unwrap().to_owned())),
            _ => Err(HandshakeError::InvalidState),
        }?;

        // C := Kdf1(C, msg.ephemeral_ct_pq)

        let ck = KDF1!(&ck, &msg.f_ephemeral_ct_pq);
        ...
        let update = match *state {
            State::InitiationSent {
                ...
                let old_sk_bytes:[u8; SIZE_EPHEMERAL_KEM_SECRET_KEY] = <[u8; SIZE_EPHEMERAL_KEM_SECRET_KEY]>::try_from(old_pq.as_ref()).unwrap();
                let eph_sk_bytes:[u8; SIZE_EPHEMERAL_KEM_SECRET_KEY] = <[u8; SIZE_EPHEMERAL_KEM_SECRET_KEY]>::try_from(eph_sk_pq.as_ref()).unwrap();
