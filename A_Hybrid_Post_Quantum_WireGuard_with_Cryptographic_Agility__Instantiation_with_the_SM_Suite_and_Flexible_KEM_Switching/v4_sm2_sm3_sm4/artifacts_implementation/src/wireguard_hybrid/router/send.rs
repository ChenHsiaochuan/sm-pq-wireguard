...
use sm4_gcm::{sm4_gcm_aad_encrypt, Sm4Key};
use spin::Mutex;
    ...
    for SendJob<E, C, T, B>
{
    ...
    fn parallel_work(&self) {
        ...
        // encrypt body
        {
            ...
            // (replaces ring's ChaCha20-Poly1305)
            let tag_offset = packet.len() - SIZE_TAG;
            let sm4_key = Sm4Key(job.keypair.send.key);
            let ct_with_tag = sm4_gcm_aad_encrypt(&sm4_key, &nonce, &[], &packet[..tag_offset]);
            debug_assert_eq!(ct_with_tag.len(), packet.len());
            packet.copy_from_slice(&ct_with_tag);
        }
