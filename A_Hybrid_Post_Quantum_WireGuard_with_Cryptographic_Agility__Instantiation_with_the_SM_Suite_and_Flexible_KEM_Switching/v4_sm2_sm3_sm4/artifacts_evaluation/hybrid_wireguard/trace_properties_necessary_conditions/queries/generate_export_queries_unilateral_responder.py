...
with open('wireguard_unilateral_responder_export_queries.spthy', 'w') as f:
	...
	f.write("query ldhi, ldhr, ldhi', ldhr', dheki, dhekr, dheki', dhekr', kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, psk':bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring;"+'\n')
	f.write("event(eRConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))) && event(eIConfirm(ck, kemltki', kemltkr, ldhi', ldhr, kempeki', dheki', dhekr', psk', h(rb), h(ra), h(re)))"+'\n')
	f.write("==> ((kemltki = kemltki') && (ldhi = ldhi'))."+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query ldhi, ldhr, ldhi', ldhr', dheki, dhekr, dheki', dhekr', kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, kempeki':bitstring, psk:bitstring, psk':bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring"
			event ="==> ((kemltki = kemltki') && (ldhi = ldhi')) || "
