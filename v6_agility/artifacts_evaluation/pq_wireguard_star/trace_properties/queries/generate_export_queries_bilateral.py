...
with open('wireguard_bilateral_export_queries.spthy', 'w') as f:
	...
	f.write("query kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring;"+'\n')
	f.write("event(eRConfirm(ck, kemltki', kemltkr, kempeki, psk, h(rb), h(ra), h(re))) && event(eHonestK(kemltkr)) && event(eHonestK(kemltki)) && event(eIConfirm(ck, kemltki, kemltkr', kempeki, psk, h(rb), h(ra), h(re)))"+'\n')
	...
	for l in sets_list:
		...
		for j in range(0, len(l)):
			...
			query = "query kemltki:bitstring, kemltkr:bitstring, kemltki':bitstring, kemltkr':bitstring, kempeki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring"
