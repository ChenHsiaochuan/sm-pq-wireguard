...
with open('wireguard_secrecy_mut7pfs_export_queries.spthy', 'w') as f:
	...
	f.write('query psk:bitstring, ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ck:bitstring;'+'\n')
	f.write('event(eRConfirm(ck, ldhi, ldhr, dheki, dhekr, psk)) && event(eIConfirm(ck, ldhi, ldhr, dheki, dhekr, psk)) && attacker(ck).'+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query psk:bitstring, ldhi:bitstring, ldhr:bitstring, dheki:bitstring, dhekr:bitstring, ck:bitstring"
			event ='==> '
