			...
			if " is true." in line:
				with open(spthyfile, 'r') as infile2:
					...
					if "eRevRb" in line:
						...
						filedata = filedata.replace("in(kb);", "let kb = h(~rb) in")

					if "eRevRa" in line:
						filedata = filedata.replace("in(~ra);", "new ~ra;")
						filedata = filedata.replace("in(ka);", "let ka = h(~ra) in")

					if "eRevRe" in line:
						filedata = filedata.replace("in(~re);", "new ~re;")
						filedata = filedata.replace("in(ke);", "let ke = h(~re) in")

					if "eRevDHE(dheki)" in line:
						filedata = filedata.replace("in(~dhekI);", "new ~dhekI;")

					if "eRevDHE(dhekr)" in line:
						filedata = filedata.replace("in(~dhekR);", "new ~dhekR;")

					if "eRevLDH(ldhi)" in line:
						filedata = filedata.replace("in(~ldhI);", "new ~ldhI;")
						filedata = filedata.replace("in(sisr);", "let sisr = (dhpkR)^~ldhI in")
						filedata = filedata.replace("in(srsi);", "let srsi = (dhpkI)^~ldhR in")

					if "eRevLDH(ldhr)" in line:
						filedata = filedata.replace("in(~ldhR);", "new ~ldhR;")
						filedata = filedata.replace("in(sisr);", "let sisr = (dhpkR)^~ldhI in")
						filedata = filedata.replace("in(srsi);", "let srsi = (dhpkI)^~ldhR in")
...
def main(profile):
    ...
    for i in range(LOG_COUNT):
        log_path = f"process_read_access/{profile}/wireguard_{profile}.pv.log"
