...
// AEAD (SM4-GCM replaces XChaCha20-Poly1305)

// MAC (HMAC-SM3 truncated, since SM3 has no native keyed mode)
use sm3::Sm3;
use hmac::Hmac;
type HMACSm3 = Hmac<Sm3>;
use subtle::ConstantTimeEq;
use crate::wireguard_hybrid::handshake::crypto_params::{HASH, MAC, SIZE_HASH, SIZE_STATIC_KEM_PUB_KEY, XOPEN, XSEAL};
use super::messages::{CookieReply, MacsFooter, TYPE_COOKIE_REPLY};
use super::types::{HandshakeError, Psk};

const LABEL_MAC1: &[u8] = b"mac1----";
const LABEL_COOKIE: &[u8] = b"cookie--";

const SIZE_COOKIE: usize = 16;
const SIZE_SECRET: usize = SIZE_HASH;
...
impl Generator {
    ...
    /// - pk_pq: The PQ public key of the peer to which the generator is associated
    ///
    /// # Returns
    ///
    /// A freshly initated generator
    pub fn new(psk: &Psk) -> Generator {
        Generator {
            mac1_key: HASH!(LABEL_MAC1, psk).into(),
            cookie_key: HASH!(LABEL_COOKIE, psk).into(),
            last_mac1: None,
            cookie: None,
        }
    }
...
pub struct Validator {
    mac1_key: [u8; 32],   // mac1 key, derived from the psk between both peers
    cookie_key: [u8; 32], // xchacha20poly key for sealing cookie response
    secret: RwLock<Secret>,
}

impl Validator {
    pub fn new(psk: &Psk) -> Validator {
        Validator {
            mac1_key: HASH!(LABEL_MAC1, psk).into(),
            cookie_key: HASH!(LABEL_COOKIE, psk).into(),
...
mod tests {
    ...
    fn new_validator_generator() -> (Validator, Generator) {
        ...
        (Validator::new(&psk), Generator::new(&psk))
    }
