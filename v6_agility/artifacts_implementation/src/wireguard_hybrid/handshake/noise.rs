...
// KEM
use oqs;
use super::agility::{self, EphemeralKemSuite};
use super::crypto_params::*;
use super::messages::*;

// HASH & MAC (SM3 replaces Blake2s)
use sm3::Sm3;
use hmac::Hmac;

// AEAD (SM4-GCM replaces ChaCha20-Poly1305) and SM4-CTR (replaces ChaCha20).
// v5: in-tree GFNI+AVX2 SM4 core with CLMUL GHASH (see wireguard_hybrid::sm_crypto).
use crate::wireguard_hybrid::sm_crypto::{
    sm4_ctr128be_apply, sm4_gcm_aad_decrypt, sm4_gcm_aad_encrypt, Sm4Key,
};
...
// HMAC hasher (HMAC-SM3)
type HMACSm3 = Hmac<Sm3>;

// Convenient alias to pass state temporarily into device.rs and back.
// v6: the ephemeral-KEM suite negotiated for this handshake travels with the
// intermediate state so the responder encapsulates with the exact algorithm the
// initiator advertised on the wire.
...
// Protocol identifier strings for SM2 + SM3 + SM4 variant.
// v6 keeps the same Noise construction string as v4/v5 — the ephemeral-KEM
// agility is negotiated by the on-wire suite byte, not baked into the protocol
// name, so the SM2/SM3/SM4 security framework and KDF chain are unchanged.
const CONSTRUCTION: &[u8] = b"Noise_IKpsk2_SM2_SM4GCM_SM3";
...
// SM4-GCM AEAD with all-zero nonce (each key is used exactly once per
// handshake step, so nonce-reuse cannot happen). Key is truncated to
// the first 16 bytes of the 32-byte KDF output.
macro_rules! SEAL {
    ($key:expr, $ad:expr, $pt:expr, $ct:expr) => {{
        let mut k16 = [0u8; 16];
        k16.copy_from_slice(&$key[..16]);
        let sm4_key = Sm4Key(k16);
        let ct_vec = sm4_gcm_aad_encrypt(&sm4_key, &ZERO_NONCE, $ad, $pt);
...
macro_rules! OPEN {
    ($key:expr, $ad:expr, $pt:expr, $ct:expr) => {{
        ...
        k16.copy_from_slice(&$key[..16]);
        let sm4_key = Sm4Key(k16);
        sm4_gcm_aad_decrypt(&sm4_key, &ZERO_NONCE, $ad, $ct)
            .map_err(|_| HandshakeError::DecryptionFailure)
            .map(|pt| $pt.copy_from_slice(&pt))
    }};
}

// SM4-CTR stream cipher (replaces raw ChaCha20). Used to wrap the
// static-KEM ciphertext field. Key is the first 16 bytes of the
// 32-byte KDF output. SM4 block size is 16 bytes, so the CTR IV is
// also 16 bytes (vs ChaCha20's 12-byte nonce).
// v5: uses the in-tree GFNI+AVX2 SM4 core (sm4_ctr128be_apply).
const ZERO_IV_SM4: [u8; 16] = [0u8; 16];
...
macro_rules! CHACHA20 {
    ($key:expr, $pt:expr) => {{
        ...
        sm4_ctr128be_apply(&k16, &ZERO_IV_SM4, &mut buffer);
        buffer
    }};
}
...
pub(super) fn create_initiation<R: RngCore + CryptoRng, O>(
    ...
    // v6: pick the active ephemeral-KEM suite and advertise it on the wire.
    let suite = agility::active_suite();
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        // v6: bind the chosen ephemeral-KEM suite into the transcript so a
        // tampered suite byte makes the two transcripts disagree (downgrade
        // resistance, in addition to the PSK-keyed mac1 over f_suite).
        let hs = HASH!(&hs, &[suite.id]);

        msg.f_type = TYPE_INITIATION;
        msg.f_sender = local; // from us
        ...
        // Ephemeral PQ KEM keys (agile algorithm)
        let kemalg = oqs::kem::Kem::new(suite.alg).unwrap();
        let (eph_pk_pq, eph_sk_pq) = kemalg.keypair().unwrap();
        let eph_pk_pq_ref = eph_pk_pq.as_ref();
        let pk_len = eph_pk_pq_ref.len();

        // msg.ephemeral := E_pub ; msg.ephemeral_pq := E_pub_pq (suite-sized prefix)
        msg.f_ephemeral = eph_pk_bytes;
        msg.f_ephemeral_pq[..pk_len].copy_from_slice(eph_pk_pq_ref);

        // C := Kdf(C, E_pub || E_pub_pq)
        let mut concat_pk = Vec::with_capacity(SIZE_SM2_POINT + pk_len);
        concat_pk.extend_from_slice(&eph_pk_bytes);
        concat_pk.extend_from_slice(eph_pk_pq_ref);
        let ck = KDF1!(&ck, &concat_pk);

        // (msg.static_ct_pq, shk1) = encapsulate(static public key of the responder)
        // The static KEM (McEliece) is the fixed identity anchor.
        ...
        // H := HASH(H, msg.ephemeral, msg.ephemeral_pq)   (suite-sized prefix)
        let hs = HASH!(&hs, &msg.f_ephemeral, &msg.f_ephemeral_pq[..pk_len]);
        ...
        *k_send_old = *k_send_old + 1;
        ...
        // update state of peer (remember which KEM suite this handshake uses)
...
pub(super) fn consume_initiation_first_part<'a, O>(
    ...
    // v6: the ephemeral-KEM suite is whatever the initiator advertised.
    let suite = msg.suite()?;
    let pk_len = suite
        .lengths()
        .map(|l| l.public_key)
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        // v6: bind the advertised ephemeral-KEM suite into the transcript
        // (mirror of create_initiation). A flipped suite byte yields a
        // different hs here than the initiator computed -> handshake fails.
        let hs = HASH!(&hs, &[suite.id]);

        // C := Kdf(C, E_pub || E_pub_pq)
        let mut concat_pk = Vec::with_capacity(SIZE_SM2_POINT + pk_len);
        concat_pk.extend_from_slice(&msg.f_ephemeral);
        concat_pk.extend_from_slice(&msg.f_ephemeral_pq[..pk_len]);
        let ck = KDF1!(&ck, &concat_pk);
        ...
        // (C, k) := Kdf2(C, DH(E_pub, S_priv) || shk1)   [static KEM = McEliece]
        let kemalg_static = oqs::kem::Kem::new(STATIC_KEM_ALG).unwrap();
...
pub(super) fn consume_initiation_second_part<'a, O>(
    ...
    // suite advertised by the initiator (validated during parse).
    let suite = msg.suite()?;
    let pk_len = suite
        .lengths()
        .map(|l| l.public_key)
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        ...
        *peer.state.lock() = State::Reset;
        ...
        // return state (to create response) — agile ephemeral KEM
        let kemalg = oqs::kem::Kem::new(suite.alg).unwrap();
        let eph_r_pk_pq = kemalg
            .public_key_from_bytes(&msg.f_ephemeral_pq[..pk_len])
            .unwrap()
            .to_owned();

        Ok((msg.f_sender, eph_r_pk, eph_r_pk_pq, hs, ck, suite))
    })
}
...
pub(super) fn create_response<R: RngCore + CryptoRng, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        // unpack state
        let (receiver, eph_r_pk, eph_r_pk_pq, hs, ck, suite) = state;

        msg.f_type = TYPE_RESPONSE;
        msg.f_sender = local;
        ...
        let eph_ct_pq_ref = eph_ct_pq.as_ref();
        let ct_len = eph_ct_pq_ref.len();
        msg.f_ephemeral_ct_pq[..ct_len].copy_from_slice(eph_ct_pq_ref);
        ...
        let mut concat_pk = Vec::with_capacity(SIZE_SM2_POINT + ct_len);
        concat_pk.extend_from_slice(&eph_pk_bytes);
        concat_pk.extend_from_slice(eph_ct_pq_ref);
        let ck = KDF1!(&ck, &concat_pk);
        ...
        let hs = HASH!(&hs, &msg.f_ephemeral, &msg.f_ephemeral_ct_pq[..ct_len]);

        // C := Kdf1(C, DH(E_priv, E_pub) || shk2)   (shk2 length depends on the suite)
        let mut concat_k = Vec::with_capacity(SIZE_DH_SHARED_SECRET + shk2.as_ref().len());
        concat_k.extend_from_slice(&shared_secret(&eph_sk, &eph_r_pk)?);
        concat_k.extend_from_slice(shk2.as_ref());
        ...
        // derive key-pair (truncate 32-byte KDF output to 16-byte SM4 key)
        let (key_recv, key_send) = KDF2!(&ck, &[]);
        let mut key_send_16 = [0u8; 16];
        let mut key_recv_16 = [0u8; 16];
        key_send_16.copy_from_slice(&key_send[..16]);
        key_recv_16.copy_from_slice(&key_recv[..16]);
...
pub(super) fn consume_response<'a, O>(
    ...
    clear_stack_on_return_fnonce(CLEAR_PAGES, || {
        // Recover the per-handshake ephemeral KEM suite from our own state, and
        // rebuild the secret key. The wire's f_suite must match what we sent.
        let (hs, ck, local, eph_sk, eph_sk_pq, suite) = {
            ...
            match *guard {
                State::InitiationSent {
                    ...
                    let suite =
                        agility::suite_by_id(suite_id).ok_or(HandshakeError::InvalidState)?;
                    if msg.f_suite != suite_id {
                        return Err(HandshakeError::InvalidState);
                    }
                    let kemalg = oqs::kem::Kem::new(suite.alg).unwrap();
                    (
        ...
        let mut concat_pk = Vec::with_capacity(SIZE_SM2_POINT + ct_len);
        concat_pk.extend_from_slice(&msg.f_ephemeral);
        concat_pk.extend_from_slice(&msg.f_ephemeral_ct_pq[..ct_len]);
        let ck = KDF1!(&ck, &concat_pk);
        ...
        let ct2 = kemalg
            .ciphertext_from_bytes(&msg.f_ephemeral_ct_pq[..ct_len])
            .unwrap()
            .to_owned();
        ...
        // C := Kdf1(C, DH(E_pub, S_priv) || shk3)   [static KEM = McEliece]
        let kemalg_static = oqs::kem::Kem::new(STATIC_KEM_ALG).unwrap();
        ...
        let update = match *state {
            State::InitiationSent {
                ...
                // Ephemeral KEM secret keys are variable length across suites;
                // compare the raw byte slices directly (same suite => same len).
                let c2 = old_pq.as_ref().ct_eq(eph_sk_pq.as_ref());
                (c1 & c2).into()
            }
            _ => false,
        };
