...
impl fmt::Display for HandshakeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            HandshakeError::InvalidHashSize => write!(f, "Hash size invalid for initiation message creation"),
        }
    }
}
...
pub enum PQError {
    ...
    // Crypto-agility (v6) errors
    UnknownKemSuite,
    KemSuiteUnavailable,
    EphemeralKemExceedsBound,
}
...
impl fmt::Display for PQError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PQError::UnknownKemSuite => write!(f, "Unknown ephemeral KEM suite (not in the agility registry)"),
            PQError::KemSuiteUnavailable => write!(f, "Requested ephemeral KEM suite is not compiled into the linked liboqs"),
            PQError::EphemeralKemExceedsBound => write!(f, "A registered ephemeral KEM exceeds the compile-time MAX_EPHEMERAL_KEM_* bound; bump the bound in agility.rs"),
        }
    }
}
