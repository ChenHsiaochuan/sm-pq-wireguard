...
use super::macs;
use super::messages::{self, CookieReply, MacsFooter, NoiseInitiation, NoiseResponse};
...
pub struct KeyState {
    pub(super) sk: DhSecretKey,    // SM2 static secret key
    pub(super) pk: DhPublicKey,    // SM2 static public key
...
impl<O> Device<O> {
    ...
    pub fn hash_static_keys(pk: &DhPublicKey, pk_pq: &oqs::kem::PublicKey) -> [u8; SIZE_HASH] {
        use super::noise::pk_to_bytes;
        let pk_bytes = pk_to_bytes(pk);
        HASH!(&pk_bytes, <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into()
    }
...
impl<O> Device<O> {
        ...
        for (pk_hash, peer) in self.pk_map.iter_mut() {
            ...
            if let Some(key) = self.keyst.as_ref() {
                if &key.pk_hash == pk_hash {
                    ...
                    let shared = diffie_hellman(key.sk.to_nonzero_scalar(), peer.pk.as_affine());
                    peer.ss = (*shared.raw_secret_bytes()).into();
                }
            } else {
                peer.ss.clear();
            }
    ...
    /// # Arguments
    ///
    /// * `sk` - (SM2 scalar representing the local private key, the kem secret key, the kem public key)
    pub fn set_sk(&mut self, sk: Option<(DhSecretKey, oqs::kem::SecretKey, oqs::kem::PublicKey)>) {
    ...
    /// # Returns
    ///
    /// The secret keys (SM2 scalar, kem secret key)
    pub fn get_sk(&self) -> Option<(&DhSecretKey, &oqs::kem::SecretKey, &oqs::kem::PublicKey)> {
        self.keyst.as_ref().map(|key| {
            (&key.sk, &key.sk_pq, &key.pk_pq)
        })
    }
    ...
    pub fn add(&mut self, pk: &DhPublicKey, pk_pq: &oqs::kem::PublicKey, opaque: O) -> Result<(), ConfigError> {
        ...
        self.pk_map.insert(
            ...
            Peer::new(
                ...
                self.keyst
                    .as_ref()
                    .map(|key| {
                        let shared = diffie_hellman(key.sk.to_nonzero_scalar(), pk.as_affine());
    ...
    pub fn begin<R: RngCore + CryptoRng>(
        ...
        match (self.keyst.as_ref(), self.pk_map.get(pk_hash)) {
            ...
            (Some(keyst), Some(peer)) => {
                ...
                let mut noise = NoiseInitiation::default();

                // create noise part of initation (stamps the active KEM suite)
                noise::create_initiation(rng, keyst, peer, local, &mut noise)?;

                // serialize compactly, then add macs over the noise body
                let mut body = noise.serialize()?;
                let mut macs = MacsFooter::default();
                peer.macs.lock().generate(&body, &mut macs);
                macs.append_to(&mut body);
    ...
    pub fn process<'a, R: RngCore + CryptoRng>(
        ...
        match LittleEndian::read_u32(msg) {
            TYPE_INITIATION => {
                // split off the mac footer and parse the (variable-length) body
                let (noise_bytes, macs) = messages::split_macs(msg)?;
                let init = NoiseInitiation::parse(noise_bytes)?;

                // consume the initiation (first part)
                let (peer, pk_hash, st_intermediate) =
                    noise::consume_initiation_first_part(self, keyst, &init)?;
                peer.macs_validator.lock().check_mac1(noise_bytes, &macs)?;

                // address validation & DoS mitigation
                if let Some(src) = src {
                    // check mac2 field
                    if !peer.macs_validator.lock().check_mac2(noise_bytes, &src, &macs) {
                ...
                let st = noise::consume_initiation_second_part(self, &init, st_intermediate, peer)?;

                // allocate new index for response
                let local = self.allocate(rng, pk_hash);

                // prepare memory for response
                let mut resp = NoiseResponse::default();

                // create response (release id on error)
                let keys = noise::create_response(rng, peer, local, st, &mut resp)
                    .map_err(|e| {
                        self.release(local);
                        e
                    })?;
                ...
                // serialize compactly, then add macs over the noise body
                let mut body = resp.serialize()?;
                let mut macs_out = MacsFooter::default();
                peer.macs.lock().generate(&body, &mut macs_out);
                macs_out.append_to(&mut body);

                // return unconfirmed keypair and the response as vector
                Ok((Some(&peer.opaque), Some(body), Some(keys)))
            }
            TYPE_RESPONSE => {
                let (noise_bytes, macs) = messages::split_macs(msg)?;
                let resp = NoiseResponse::parse(noise_bytes)?;

                // check mac1 field
                // retrieve peer and copy initiation state
                let peer = self.lookup_id(resp.f_receiver)?;
                peer.macs_validator.lock().check_mac1(noise_bytes, &macs)?;
                ...
                noise::consume_response(keyst, &resp, peer)
            }
            TYPE_COOKIE_REPLY => {
...
mod tests {
    ...
    // Verify that each peer has a unique pre-computed shared secret.
    // (SM2 keys must be generated properly; cannot use arbitrary bytes like X25519.)
    #[test]
    fn unique_shared_secrets() {
        let kemalg = oqs::kem::Kem::new(STATIC_KEM_ALG).unwrap();

        let sk  = DhSecretKey::random(&mut OsRng08);
        let pk1 = DhSecretKey::random(&mut OsRng08).public_key();
        let pk2 = DhSecretKey::random(&mut OsRng08).public_key();

        let (pk_pq,  sk_pq)  = kemalg.keypair().unwrap();
        let (pk1_pq, _sk1_pq) = kemalg.keypair().unwrap();
        let (pk2_pq, _sk2_pq) = kemalg.keypair().unwrap();

        let mut dev: Device<u32> = Device::new();
        dev.set_sk(Some((sk, sk_pq, pk_pq)));

        dev.add(&pk1, &pk1_pq, 1).unwrap();
        // pk2 is random and almost certainly different; adding should succeed
        let _ = dev.add(&pk2, &pk2_pq, 2);

        // Every shared secret should be unique
