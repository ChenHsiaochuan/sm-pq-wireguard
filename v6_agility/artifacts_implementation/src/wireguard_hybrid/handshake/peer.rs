...
use oqs;
use crate::wireguard_hybrid::handshake::crypto_params::{HASH, SIZE_HASH, SIZE_STATIC_KEM_PUB_KEY};
...
pub(super) struct Peer<O> {
    ...
    // constant state
    pub ss: [u8; 32], // precomputed DH(static, static) — SM2 ECDH output
    pub psk: Psk,     // psk of peer
    pub pk: DhPublicKey, // SM2 static public key
...
pub enum State {
    ...
    InitiationSent {
        local: u32,
        eph_sk: DhSecretKey,          // SM2 ephemeral secret key
        eph_sk_pq: oqs::kem::SecretKey,
        // v6: the ephemeral-KEM suite chosen for this handshake, so the
        // response can be decapsulated with the matching algorithm.
        suite_id: u8,
...
impl Drop for State {
    fn drop(&mut self) {
        if let State::InitiationSent { hs, ck, .. } = self {
            // eph_sk implements ZeroizeOnDrop (sm2::SecretKey)
            hs.clear();
            ck.clear();
        }
    }
...
impl<O> Peer<O> {
    pub fn new(pk: DhPublicKey, pk_pq: oqs::kem::PublicKey, own_pk_pq: &oqs::kem::PublicKey, ss: [u8; 32], opaque: O) -> Self {
        ...
        Self {
            ...
            ss,
            hash_static_pq_send: HASH!(<[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(own_pk_pq.as_ref()).unwrap(), <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into(),
            hash_static_pq_recv: HASH!(<[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap(), <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(own_pk_pq.as_ref()).unwrap()).into(),
            psk: [0u8; 32],
            pk_hash: HASH!(&pk_bytes, <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into(),
            pk,
            pk_pq,
            k_send: Mutex::new(0),
            k_recv: Mutex::new(0),
