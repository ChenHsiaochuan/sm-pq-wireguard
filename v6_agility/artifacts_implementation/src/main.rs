...
#[cfg(any(all(feature = "hybrid", feature = "post_quantum")))]
compile_error!("Features hybrid and post_quantum are mutually exclusive and cannot be enabled together");
...
#[cfg(all(not(feature = "hybrid"), not(feature = "post_quantum")))]
...
fn main() {
    ...
    let mut payload_bench: u32 = 0;
    let mut kem_summary: u32 = 0;
    ...
    while i < args.len(){
        ...
        else if args[i].eq("--payload-bench") | args[i].eq("-pb") {
            if i+1 == args.len() {
                panic!("nb. of executions is required when running the payload benchmark.");
            }
            match args[i+1].parse::<u32>() {
                Ok(n) => payload_bench = n,
                Err(_) => panic!("--payload-bench requires an integer nb. of handshake iterations."),
            }
            i += 2
        }
        else if args[i].eq("--kem-summary") | args[i].eq("-ks") {
            if i+1 == args.len() {
                panic!("nb. of rounds is required for --kem-summary.");
            }
            match args[i+1].parse::<u32>() {
                Ok(n) => kem_summary = n,
                Err(_) => panic!("--kem-summary requires an integer nb. of rounds."),
            }
            i += 2
        }
        else {
    ...
    // ---------------------------------------------------------------------
    // Crypto agility (v6): pick the ephemeral post-quantum KEM at start-up
    // through the single external interface ($WG_KEM_SUITE), and bail out early
    // if a registered suite outgrew its compile-time size bound. Selection can
    // also be changed live at run time via the agility API without restarting.
    // ---------------------------------------------------------------------
    #[cfg(feature = "hybrid")]
    {
        use wireguard_hybrid::handshake::agility;
        if let Err(e) = agility::validate_registry() {
            eprintln!("KEM agility registry invalid: {}", e);
            exit(-6);
        }
        match agility::init_from_env() {
            Ok(suite) => eprintln!(
                "Crypto agility: ephemeral KEM = {} [{}] (set {}=<token> to change; available: {})",
                suite.token,
                suite.label,
                agility::ENV_VAR,
                agility::available_suites()
                    .iter()
                    .map(|s| s.token)
                    .collect::<Vec<_>>()
                    .join(", "),
            ),
            Err(msg) => eprintln!("Crypto agility: {}", msg),
        }
    }
    ...
    {
        if payload_bench > 0 {
            wireguard_hybrid::benchs::benchmark_payload_latency(payload_bench);
            return;
        }
        if kem_summary > 0 {
            wireguard_hybrid::benchs::benchmark_kem_summary(kem_summary);
            return;
        }
    }
    ...
    {
        eprintln!("TUN device mode is only supported on Linux.");
        exit(-1);
    }
