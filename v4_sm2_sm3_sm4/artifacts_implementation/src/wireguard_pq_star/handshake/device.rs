...
impl<O> Device<O> {
    ...
    pub fn hash_static_keys(pk_pq: &oqs::kem::PublicKey) -> [u8; SIZE_HASH] {
        HASH!(<[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pk_pq.as_ref()).unwrap()).into()
    }
...
impl<O> Device<O> {
    ...
    pub fn set_sk(&mut self, sk: Option<( oqs::kem::SecretKey, oqs::kem::PublicKey)>) {
        // update secret and public key
        self.keyst = sk.map(|sk| {
            // let macs = macs::Validator::new(&pk, &sk.2);

            KeyState {
                pk_hash: Device::<O>::hash_static_keys(&sk.1),
                sk_pq: sk.0,
                pk_pq: sk.1,
                // macs,
            }
    ...
    pub fn get_sk(&self) -> Option<(&oqs::kem::SecretKey, &oqs::kem::PublicKey)> {
...
//             dev.set_sk(Some((sk_pq, pk_pq)));
//
//             let hash1 = Device::<u32>::hash_static_keys(&pk1_pq);
//
//             dev.add(&pk1_pq,&pk2_pq, 1).unwrap();
//             if dev.add(&pk2_pq, &pk1_pq, 0).is_err() {
