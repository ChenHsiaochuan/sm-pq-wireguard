...
use sm4_gcm::{sm4_gcm_aad_decrypt, Sm4Key};
use spin::Mutex;
    ...
    fn parallel_work(&self) {
        ...
        // decrypt
        {
            ...
            let ok = (|| {
                ...
                let plaintext = match sm4_gcm_aad_decrypt(&sm4_key, &nonce, &[], packet) {
                    Ok(pt) => pt,
                    Err(_) => return false,
                };
                // overwrite ciphertext+tag region with plaintext (zero-pad tag area)
                packet[..plaintext.len()].copy_from_slice(&plaintext);
                for b in &mut packet[plaintext.len()..] {
                    *b = 0;
                }
