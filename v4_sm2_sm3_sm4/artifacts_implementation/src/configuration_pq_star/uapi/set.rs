...
use crate::wireguard_pq_star::handshake::crypto_params::{SIZE_STATIC_KEM_PUB_KEY, SIZE_STATIC_KEM_SECRET_KEY, STATIC_KEM_ALG};
...
impl<'a, C: Configuration> LineParser<'a, C> {
    ...
    fn new_peer(value: &str) -> Result<ParserState, ConfigError> {
        ...
        match Vec::from_hex(value) {
            Ok(pkv) => Ok({
                let k: [u8; SIZE_STATIC_KEM_PUB_KEY] = <[u8; SIZE_STATIC_KEM_PUB_KEY]>::try_from(pkv).unwrap();
                let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[..].try_into().unwrap();
    ...
    pub fn parse_line(&mut self, key: &str, value: &str) -> Result<(), ConfigError> {
        ...
        fn flush_peer<C: Configuration>(config: &C, peer: &ParsedPeer) -> Option<ConfigError> {
            ...
            if peer.remove {
                ...
                let pk_hash = Device::<u32>::hash_static_keys(&peer.public_key_pq);
            ...
            if !peer.update_only {
                ...
                let pk_hash = Device::<u32>::hash_static_keys(&peer.public_key_pq);
                config.add_peer(&peer.public_key_pq, &pk_hash);
            }
        ...
        match self.state {
            ...
            ParserState::Interface => match key {
                ...
                "private_key" => match Vec::from_hex(value) {
                    Ok(skv) => {
                        let k = <[u8; SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]>::try_from(skv).unwrap();

                        self.config.set_private_key(if k.ct_eq(&[0u8; SIZE_STATIC_KEM_SECRET_KEY+SIZE_STATIC_KEM_PUB_KEY]).into() {
                            None
                        } else {
                            let sk_pq: [u8; SIZE_STATIC_KEM_SECRET_KEY] = k[..SIZE_STATIC_KEM_SECRET_KEY].try_into().unwrap();
                            let pk_pq: [u8; SIZE_STATIC_KEM_PUB_KEY] = k[SIZE_STATIC_KEM_SECRET_KEY..].try_into().unwrap();
                ...
                "replace_peers" => match value {
                    "true" => {
                        for p in self.config.get_peers() {
                            let pk_hash = Device::<u32>::hash_static_keys(&p.public_key_pq);
