...
with open('wireguard_secrecy_mut7_export_queries.spthy', 'w') as f:
	...
	f.write('event(eRConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))) && event(eIConfirm(ck, kemltki, kemltkr, ldhi, ldhr, kempeki, dheki, dhekr, psk, h(rb), h(ra), h(re))) && attacker(ck).'+'\n\n')
