...
with open('wireguard_agreement_confirm_export_queries.spthy', 'w') as f:
	...
	f.write('query kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, dheki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring;'+'\n')
	f.write('event(eRConfirm(ck, kemltki, kemltkr, tpkr, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re)))'+'\n')
	f.write('==> event(eIConfirm(ck, kemltki, kemltkr, tpki, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))).'+'\n\n')

	for l in sets_list:

		for j in range(0, len(l)):

			query = "query kemltki:bitstring, kemltkr:bitstring, tpki:bitstring, tpkr:bitstring, kempeki:bitstring, dheki:bitstring, psk:bitstring, ck:bitstring, rb:bitstring, ra:bitstring, re:bitstring"
			event ='==> event(eIConfirm(ck, kemltki, kemltkr, tpki, kempeki, psk, h((rb, tpki)), h((ra, tpkr)), h(re))) || '
...
with open('wireguard_agreement_confirm_export_queries.spthy', 'r') as f:
	fdata = f.read()
